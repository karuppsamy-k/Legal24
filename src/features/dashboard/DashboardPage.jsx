...
const ANALYTICS_DATA = {
  ...
  advocate: {
    monthly: {
      labels: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'],
      datasets: [
        { label: 'Active Advocates', data: [30, 35, 40, 42, 45, 48, 50, 55, 60, 65, 70, 75], color: '#4ce1b1', percentage: 65, trend: '+2.10%' },
        { label: 'Pending Approval', data: [10, 12, 8, 15, 5, 3, 5, 8, 10, 5, 2, 4], color: '#ffd68a', percentage: 15, trend: '-0.50%' },
        { label: 'Busy', data: [15, 18, 22, 20, 25, 30, 28, 32, 35, 40, 42, 45], color: '#6c9cff', percentage: 20, trend: '+1.20%' }
      ]
    },
    yearly: {
      labels: ['2015', '2016', '2017', '2018', '2019', '2020', '2021', '2022', '2023', '2024'],
      datasets: [
        { label: 'Active Advocates', data: [80, 120, 180, 250, 340, 450, 580, 720, 880, 1100], color: '#4ce1b1', percentage: 72, trend: '+4.50%' },
        { label: 'Pending Approval', data: [20, 30, 25, 40, 50, 60, 40, 30, 20, 15], color: '#ffd68a', percentage: 10, trend: '-1.80%' },
        { label: 'Busy', data: [40, 60, 80, 120, 180, 250, 320, 400, 480, 600], color: '#6c9cff', percentage: 18, trend: '+3.40%' }
      ]
    }
  },
  case: {
...
const AnalyticsChart = () => {
  ...
  const rawData = ANALYTICS_DATA[activeModule][timeRange];
    ...
    <div className="analytics-container">
      <div className="analytics-header">
        <div className="module-selector">
          ...
          <button className={`module-btn ${activeModule === 'advocate' ? 'active' : ''}`} onClick={() => setActiveModule('advocate')}>
            <Briefcase size={16} /> Advocates
          </button>
          <button className={`module-btn ${activeModule === 'case' ? 'active' : ''}`} onClick={() => setActiveModule('case')}>
            <FileText size={16} /> Cases
          </button>
        </div>
...
export default function DashboardPage() {
  ...
  // Handle loading state gracefully while keeping UI intact
  const { stats, pulse, auditFeed, applications, users } = data || { 
    stats: [
      { title: "Loading...", value: "...", detail: "Please wait", badge: "" },
      { title: "Loading...", value: "...", detail: "Please wait", badge: "" },
      { title: "Loading...", value: "...", detail: "Please wait", badge: "" }
    ], 
    pulse: { activeCases: "...", pendingApprovals: "...", bars: [] }, 
    auditFeed: [],
    applications: [],
    users: []
  };
