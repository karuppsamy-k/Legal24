...
import { clientChatFlow, ADVOCATES_DATA } from './clientChatbotModel';
import './clientChatbot.css';

// ── Helper: filter & sort advocates ────────────────────────
function filterAdvocates(filters) {
  ...
  // Sort by rating (high to low) takes priority
...
// ═══════════════════════════════════════════════════════════
// Main ClientChatBot Component
// ═══════════════════════════════════════════════════════════
export default function ClientChatBot() {
  const [isOpen, setIsOpen] = useState(false);
  const [history, setHistory] = useState([]);
  const [isTyping, setIsTyping] = useState(false);
  const [context, setContext] = useState({});       // tracks caseType, intent, mode, urgency
  ...
  useEffect(() => {
    if (isOpen && history.length === 0) {
      ...
      setTimeout(() => {
        setIsTyping(false);
        setHistory([{ type: 'bot', node: clientChatFlow.start }]);
      }, 600);
    }
  }, [isOpen]);
  ...
  const handleOptionClick = useCallback((option) => {
    ...
    if (option.action === 'SHOW_ALL_ADVOCATES') {
      ...
      setTimeout(() => {
        ...
        setHistory(prev => [...prev, { type: 'bot', node: clientChatFlow.results_shown }]);
      }, 800);
      return;
    }

    // Navigate to next node
    if (option.next && clientChatFlow[option.next]) {
      setIsTyping(true);
      setTimeout(() => {
        setIsTyping(false);
        setHistory(prev => [...prev, { type: 'bot', node: clientChatFlow[option.next] }]);
      }, 700);
    }
  }, [navigate]);
  ...
  const handleBookAdvocate = (advocate) => {
    ...
    setTimeout(() => {
      setIsTyping(false);
      setHistory(prev => [...prev, { type: 'bot', node: clientChatFlow.booked }]);
      setTimeout(() => navigate('/client-consultations'), 1500);
    }, 800);
  };
  ...
  return (
    <div className="client-chatbot-wrapper">
      {/* ── FAB Toggle ── */}
      {!isOpen && (
        <button className="client-chatbot-fab" onClick={() => setIsOpen(true)} id="client-chatbot-toggle">
          <Sparkles size={24} />
        </button>
      )}
            ...
            {history.map((msg, idx) => {
              ...
              return (
                <div key={idx} style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  ...
                  {/* Show options only for the last bot message, and only if we're not showing filters/results */}
